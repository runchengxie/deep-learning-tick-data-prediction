...
| L2 事件流 Transformer | 无损归并的委托、成交和快照事件 | 是 | 100M 三 seed 与冻结表征对照已完成，联合微调入口已实现 | HGB 组合输入出现稳定 Rank IC 增量，交易价值仍需跨窗口复核 |
...
主要限制：训练需要远端 GPU，存储与输入吞吐成本都很高。100M 最近折三 seed 的 H5 validation Rank IC 均值为 0.07259，OOS 均值为 0.04300，三组方向均为正。冻结表征接入 HGB 后，三 seed 预测均值的 OOS Rank IC 从分钟基线的 0.04010 提高到 0.05701，验证月也从 0.01808 提高到 0.02833。Top-100 成本后主动收益仍为负，当前证据只覆盖一个验证月和一个 OOS 月。
...
4. 下一步补充风险暴露，运行一个 seed 的联合端到端小实验，并在额外时间窗口复核增量。
5. `probe150m` 等待联合实验和跨窗口结果。
...
分钟链路暂不新增独立 Transformer。联合端到端实验会复用现有事件流 Transformer，将分钟特征与日级隐藏表示送入同一个预测头。第一轮固定 100M checkpoint、股票日交集和日期，只检验联合优化能否超过当前 frozen E2。现有 TCN 的验证优势没有稳定泛化，M3 和 frozen E2 也尚未形成跨月稳定的成本后主动收益，因此先做一个 seed 的受限实验。
