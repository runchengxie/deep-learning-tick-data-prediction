...
| L2 事件流 Transformer | 无损归并的委托、成交和快照事件，可拼接分钟特征 | 是 | 100M 三 seed、冻结表征对照和联合 seed 0 已完成 | 联合训练继续提高 Rank IC，头部命中和成本后主动收益仍需优化 |
...
主要限制：训练需要远端 GPU，存储与输入吞吐成本都很高。100M 最近折三 seed 的 H5 validation Rank IC 均值为 0.07259，OOS 均值为 0.04300，三组方向均为正。冻结表征接入 HGB 后，三 seed 预测均值的 OOS Rank IC 从分钟基线的 0.04010 提高到 0.05701，验证月也从 0.01808 提高到 0.02833。联合 seed 0 进一步得到 0.05784 validation Rank IC 和 0.06296 OOS Rank IC。OOS `Precision@100` 为 0.24762，Top-100 日均成本后主动收益为 -9.26bp。当前证据只覆盖一个验证月、一个 OOS 月和一个联合训练 seed。
...
正式 seed 0 在第 2 个 epoch 取得最佳 validation Rank IC，后续两轮回落并触发早停。这表明一次低学习率主干更新能够提供增量，继续训练会较快过拟合。联合模型的 OOS `NDCG@100` 为 0.54452，与冻结 HGB 三 seed 预测均值接近。日均单边换手降至 49.91%，头部命中率和成本后主动收益同步下降。后续实验需要同时观察 Rank IC、Precision 和交易成本，不能只按 Rank IC 决定容量扩张。
...
4. 联合端到端 seed 0 已完成，validation Rank IC 为 0.05784，OOS Rank IC 为 0.06296。头部命中和成本后主动收益没有同步改善。
5. 下一步补充风险暴露、联合 seed 1 和 2，并在额外时间窗口复核增量。
6. `probe150m` 等待多 seed、跨窗口和交易指标结果。
...
分钟链路暂不新增独立 Transformer。联合端到端实验已经复用现有事件流 Transformer，将分钟特征与日级隐藏表示送入同一个预测头。seed 0 的 Rank IC 增量支持继续做相同配置的 seed 1 和 2。现有 TCN 的验证优势没有稳定泛化，M3、frozen E2 和联合 seed 0 也没有形成跨月稳定的成本后主动收益，因此当前重点是稳健性和交易目标，暂不增加模型容量。
