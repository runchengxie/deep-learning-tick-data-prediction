...
| L2 事件流 Transformer | 无损归并的委托、成交和快照事件 | 是 | 数据打包和 100M 输入基准已完成 | 正式训练尚未开始，当前只有工程结论 |
| FI-2010 DeepLOB | FI-2010 十档订单簿窗口 | 是 | 已归档到 `legacy/` | 用于论文复现和兼容检查，不代表 A 股次日预测效果 |
| LambdaMART | 按交易日分组的横截面特征 | 否 | M4 规划项，尚未实现 | 用于直接优化同一天股票之间的排序 |
...
主要限制：训练需要远端 GPU，存储与输入吞吐成本都很高。固定窗口物化和可恢复训练工作流已实现，真实 seed 0 仍在基础设施验收阶段，尚无正式训练结果。100M seed 0 通过门槛后才补 seed 1、2，并决定是否运行 150M 容量消融。
...
LambdaMART 是基于梯度提升树的排序模型，属于传统机器学习模型。它已经写入 M4 路线图，仓库目前还没有正式实现。
...
主要限制：需要严格正确的日期分组和无泄漏特征。它可以改善排序目标的匹配程度，无法补充输入特征中原本不存在的信息。M4 会在相同数据指纹和评估口径下与 HGB 比较。

## 是否继续加入 Transformer

项目已经拥有一条完整的 Transformer 模型代码链路，下一步应先验证现有事件流模型。建议顺序如下：

1. 完成固定窗口缓存的真实 seed 0 物化、上传和短恢复验收，再运行 `capacity100m` 最近折 seed 0。
2. seed 0 达到预设信号与稳定性门槛后补 seed 1 和 2。
3. 把通过门槛的 checkpoint 冻结成 embedding，接入 M4 的 HGB 或 LambdaMART，检查它能否提供分钟特征之外的增量。
4. 只有 100M 信号或冻结 embedding 增量通过门槛，才运行 `probe150m` 容量消融。
