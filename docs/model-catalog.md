...
主要限制：训练需要远端 GPU、稳定的数据暂存和可恢复工作流，存储与输入吞吐成本都很高。当前只完成数据打包、输入优化和 A100 基准，尚无正式 seed 结果。100M seed 0 通过门槛后才补 seed 1、2，并决定是否运行 150M 容量消融。
...
1. 补齐月度流式暂存、正式训练、产物回传和断点续跑，运行 `capacity100m` 最近折 seed 0。
