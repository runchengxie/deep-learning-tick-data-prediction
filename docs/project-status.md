...
1. 在额外滚动窗口复核低换手规则能否稳定改善成本后主动收益
2. 实现绝对开仓门槛、换仓收益差门槛和现金仓位
3. 测试 1、3、5 日调仓以及错峰组合
4. 引入按股票流动性变化的动态成本，并补齐行业、规模、波动率和流动性暴露
5. 多窗口结果稳定后，再评估同时约束净收益、换手和风险的训练目标
6. 上述交易门槛通过后，再评估 `probe150m` 的预算和 seed 0 门槛
