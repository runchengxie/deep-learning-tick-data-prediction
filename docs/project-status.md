...
| L2 事件流 | 三流无损打包、因果 Transformer、训练恢复、尾盘缓存、冻结 embedding 和联合微调 | 100M 最近折三 seed、冻结表征对照和联合 seed 0 已完成 | 联合信号待多 seed 和跨窗口复核 |
...
`FEAT-EVENTSTREAM-JOINT-001` seed 0 已完成。轻量缓存含 22,409 个训练样本、6,963 个 validation 样本和 8,125 个 OOS 样本，共 17,948,094 字节，数据指纹为 `e4f54a62e4be3f36ac0693db59ebcdb120cd753d2dc36415b8686adaa13c1bb6`。它只保存 120 维分钟特征、目标和共享尾盘缓存行号，没有复制 6.17 GiB 的事件数组。缓存与 Drive 副本的 5 个文件已逐项核对。

联合模型载入 seed 0 的 100M best checkpoint。第 1 个 epoch 固定 Transformer，第 2 个 epoch 解冻主干后，validation Rank IC 从 0.04430 提高到 0.05784。第 3、4 个 epoch 分别回落到 0.01457 和 0.03628，训练按两轮耐心值提前停止，最终使用第 2 个 epoch。OOS Rank IC 为 0.06296，高于冻结 HGB seed 0 的 0.04333，也高于冻结 HGB 三 seed 预测均值的 0.05701。OOS `NDCG@100` 为 0.54452，`Precision@100` 为 0.24762，Top-100 日均单边换手为 49.91%，日均成本后主动收益为 -9.26bp。联合训练提高了横截面相关性并降低了换手，头部命中率和成本后主动收益没有同步改善。`probe150m` 继续等待多 seed、额外时间窗口和交易目标优化的证据。
...
2. 为联合训练补齐 seed 1、2 的精确 checkpoint 身份和配置覆盖，再运行两组重复实验
3. 在额外时间窗口复核 frozen E2 与联合模型的 Rank IC、NDCG、Precision 和成本后主动收益
4. 根据联合模型的头部命中率和成本结果，评估小规模横截面排序损失或成本感知选模
5. 多 seed 与跨窗口证据通过后，再评估 `probe150m` 的预算和 seed 0 门槛
