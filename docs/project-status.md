...
| L2 事件流 | 三流无损打包、因果 Transformer、训练恢复、尾盘缓存、冻结 embedding、联合微调和多任务梯度审计 | 半衰期、H5 错峰持有、排名平滑和已知风险暴露已检查 | 梯度审计入口已完成，正式两折结果待运行 |
...
1. 在最近折和相邻折的 validation 各审计 16 个固定 batch，对比初始化与 best checkpoint 的梯度范数和夹角
2. 日级梯度明显偏弱时，比较原始收益与每日截面去极值 z 标签
3. 日级梯度强度正常时，比较全部位置、最后位置和尾部加权监督。持续任务冲突还要同步复核任务权重
4. seed 0 在两折的 validation、OOS 和头部指标同时改善后，再补 seed 1、2、更多滚动窗口和排序目标
