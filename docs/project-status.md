...
本机没有可用 CUDA GPU。Google Drive 总额为 200 GiB。seed 共用的尾盘窗口缓存已完成本地生成、全量核对和远端上传，包含 39,903 个股票日、5 个分片，共 6,619,831,094 字节，约 6.17 GiB，数据指纹为 `59577182c8124c312de0591059c67e55d472511ca77753403ce77afbf8f109f4`。上传后 Drive 剩余约 15.2 GiB。三个 checkpoint 将分别导出 960 维向量，输出体积远小于训练缓存。

冻结 embedding 下游对照已经实现。E0 使用分钟特征，E1 使用冻结 embedding，E2 拼接两类特征。三组输入分别交给 HGB 和 LambdaMART，统一使用次日 open-to-following-open 标签、最近折日期、股票交集和评估口径。三个 checkpoint 分别训练下游模型，最后可以平均下游预测。代码不会逐维平均三个 embedding。共享尾盘缓存已经就绪，三个 seed 的 embedding 与下游结果仍待生成，`probe150m` 暂缓。
...
1. 用三个 100M checkpoint 分别导出 960 维冻结 embedding
2. 运行 E0、E1、E2 的 HGB 与 LambdaMART 对照，汇总 Rank IC、NDCG、Precision、Top-K 成本后收益和月度稳定性
3. 补齐行业、规模、波动率和流动性暴露输入，区分新增信息与风险暴露代理
4. 根据三个 seed 的配对增量和预测组合结果决定是否运行 `probe150m`
