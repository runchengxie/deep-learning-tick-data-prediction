...
| L2 事件流 | 三流无损打包、因果 Transformer、训练恢复、尾盘缓存、冻结 embedding 和联合微调 | 半衰期、H5 错峰持有、排名平滑和已知风险暴露已检查 | 跨窗口交易门槛未通过 |
...
1. 记录多任务对共享主干的梯度范数和夹角，确认日级任务的真实训练强度
2. 比较原始收益与每日截面去极值 z 标签
3. 比较全部位置、最后位置和尾部加权监督
4. 单 seed 同时改善 validation 和相邻 OOS 后，再补 seed 1、2、更多滚动窗口和排序目标
