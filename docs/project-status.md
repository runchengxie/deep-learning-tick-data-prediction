...
| L2 事件流 | 三流无损打包、窗口采样、因果 Transformer、训练恢复和预测导出 | 2025 年 8 月至 12 月共 103 个交易日已打包，A100 输入基准已完成 | 正式训练待执行 |
...
事件流目前确认了数据打包完整性和输入吞吐，尚无正式模型 seed 结果。AgentX 的 M3 仍在准备正式输入，也尚无本阶段的 Top-K 策略结论。
...
2. 为 `1M/raw-200` 固定候选配置和锁定测试触发条件，在满足条件前保持 2025 隔离
3. 根据存储和训练预算安排事件流最近折的正式 seed 0，随后再决定是否补 seed 1 和 2
4. 所有新结论写入[实验日志](research/experiment-log.md)，并同步更新本页的状态摘要
