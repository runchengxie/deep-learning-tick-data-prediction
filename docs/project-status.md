...
| L2 事件流 | 三流无损打包、因果 Transformer、训练恢复、尾盘缓存、冻结 embedding、联合微调和多任务梯度审计 | 两折梯度审计确认日级任务在 best checkpoint 明显偏弱 | 标签尺度 seed 0 两折实验待运行 |
...
1. 在最近折和相邻折运行 seed 0 的每日截面去极值 z 标签实验
2. 两折的 validation、OOS 和极端组收益差同时改善后，再补 seed 1、2
3. 标签尺度未形成跨折增量时，比较全部位置、最后位置和尾部加权监督
4. 训练机制形成稳定增量后，再补更多滚动窗口和横截面排序目标
