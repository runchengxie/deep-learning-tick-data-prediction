...
| L2 事件流 | 三流无损打包、因果 Transformer、训练恢复、尾盘缓存、冻结 embedding 和联合微调 | 100M 最近折三 seed 与冻结表征对照已完成 | 联合实验基础设施已就绪，seed 0 待运行 |
| AgentX 研究闭环 | 提案、批评、白名单执行器、Registry、审计、比较和锁定测试审批 | M0 至 M3 已完成，M4 与 M5 最近折对照已落地 | M4、M5 进行中 |
...
LambdaMART 的 E2 三 seed 与跨月结果波动较大。三 seed 预测均值的 validation Rank IC 从 -0.04334 降到 -0.05081，OOS 从 0.00766 提高到 0.01389。E1 预测均值的 OOS Rank IC 为 0.03414，Top-100 日均成本后主动收益为 15.53bp，但 validation 对应收益为 -15.34bp。当前证据支持保留冻结表征和 HGB 融合候选，并开展受限的联合端到端实验。交易价值、跨窗口稳定性和风险归因仍需补证。行业、规模、波动率和流动性暴露输入尚未提供，结果已标记为 `unavailable`。`probe150m` 继续暂缓。

`FEAT-EVENTSTREAM-JOINT-001` 已具备可运行入口。轻量缓存只保存 120 维分钟特征、目标和共享尾盘缓存行号，不复制 6.17 GiB 的事件数组。联合模型载入 seed 0 的 100M best checkpoint，将最后一个有效事件的隐藏状态与分钟特征塔输出拼接。第一轮先冻结 Transformer 一轮，再用较小学习率解冻主干。股票日交集、日期、标签、Rank IC、NDCG、Precision 和 Top-K 成本口径均与 frozen E2 保持一致。正式缓存、远端训练和结果尚未完成。
...
2. 固定当前折配置，增加事件流与分钟特征联合训练的小规模端到端实验，先运行一个 seed
3. 在额外时间窗口复核 HGB E2 的 Rank IC 和成本后主动收益，避免只依赖 2025 年 11 月和 12 月
