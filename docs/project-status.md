...
本页汇总 2026-08-20 可以由代码、测试或实验产物核对的项目事实。路线图记录研究设计，实验日志保留历史数字，本页负责说明当前进度。状态变化时优先更新本页，再检查根目录 README 和相关专题文档。
...
| L2 事件流 | 三流无损打包、因果 Transformer、训练恢复、尾盘缓存、冻结 embedding、联合微调、多任务梯度审计和标签覆盖层 | 标签尺度 seed 0 在两折提高 Rank IC，头部收益没有全部通过跨折门槛 | 转入监督位置消融 |
...
`EVT-LABEL-SCALE-001` 已完成两折 seed 0。最近折的 validation 与 OOS Rank IC 从 0.04345、0.05879 提高到 0.11747、0.07446，相邻折从 0.08735、0.03305 提高到 0.13534、0.07755。相邻折的 OOS 极端组收益差从 -0.51240% 提高到 0.48367%，最近折从 0.34105% 降到 0.08885%。预注册的两折全指标改善门槛没有通过，因此暂不补 seed 1、2。监督位置实验入口已完成，下一步在两折运行 `last` 和 `tail_weighted` 的 seed 0 validation 对照。完整结果见[事件流标签尺度实验](research/eventstream-label-scale.md)。
...
1. 实现全部位置、最后位置和线性尾部加权三种日级监督模式
2. 复用现有 z 标签全部位置结果，只运行后两种模式的两折 seed 0 validation
3. 按预注册 validation 门槛选出一个方案，再开放两折 OOS
4. 两折的 Rank IC 和极端组收益差同时改善后，再补 seed 1、2
5. 没有方案通过时，检查日级任务权重和成本感知排序目标
6. 获得带日期的行业分类数据后，补齐行业暴露归因
7. 成本后主动收益形成跨窗口增量后，再评估 `probe150m` 的预算和 seed 0 门槛
8. 所有新结论写入[实验日志](research/experiment-log.md)，并同步更新本页的状态摘要
