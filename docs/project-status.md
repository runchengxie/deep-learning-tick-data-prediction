...
| L2 事件流 | 三流无损打包、窗口采样、因果 Transformer、训练恢复、存储预检和预测导出 | 固定窗口物化与正式训练工作流已实现，真实 seed 0 物化和恢复验收待完成 | T0 基础设施验收中 |
...
事件流目前确认了数据打包完整性和输入吞吐，尚无正式模型 seed 结果。本机没有可用 CUDA GPU。Google Drive 总额为 200 GiB，当前约使用 98.1 GiB，剩余约 100.5 GiB，只保存了 2025 年 8 月的 68.58 GB benchmark pack。完整五个月 pack 约为 313.11 GiB，无法整体放入现有 Drive。

正式训练采用每个 seed 约 25 GiB 的固定窗口缓存。缓存保留 float32 特征和原始标签，并绑定源清单、配置、日期、seed 与逐文件 SHA-256。Colab 工作流已经支持完整预检、checkpoint 恢复、失败产物回传和 OOS 访问控制。真实 seed 0 仍需完成缓存生成、上传和短恢复验收。验收通过后运行正式 seed 0，根据预设门槛决定是否补 seed 1 和 2，再把冻结 embedding 接入 M4。`probe150m` 只在 100M 信号或冻结 embedding 迁移门槛通过后用于受控容量消融。
...
3. 验收通过后，在远端 GPU 运行最近折 `capacity100m` seed 0，通过预设门槛后补 seed 1 和 2
