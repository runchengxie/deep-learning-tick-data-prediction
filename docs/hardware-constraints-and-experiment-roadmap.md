...
当前优先验证深度模型直接接受 tick/十档盘口并输出次日信号的端到端能力。因此工程顺序为：

```text
原始 snapshot 200 tick 端到端 MVP
  → Colab 吞吐和断点续训
  → 2021 至 2023 训练 / 2024 验证 / 2025 锁定测试
  → 分钟缓存与 Logistic 作为内部对照
  → 500 / 1,000 tick 增量实验
```

这不取消小模型对照，只调整优先级，先打通可复现的端到端链路。当前模型使用共享 DeepLOB
编码 2 个 100-event 块，GRU 汇总后同时输出连续超额收益分数和三分类概率，两个损失共同
反向传播到原始盘口卷积层。

raw-200 同时保留两个容量档位：86,775 参数默认基线，以及 1,033,383 参数容量扩展。扩展版
将卷积通道、Inception 分支和日内 embedding 一起加宽，用于检验模型容量是否为瓶颈；它不因
参数量更大而自动成为主模型。
...
同一数据盘还包含 2021 至 2025 的 60 个月 order 和 trades 目录，分别约 1.2 TB 和
...
原始盘口是当前端到端主模型，同时回答：分钟聚合是否丢失了可用于次日预测的信息。
...
`configs/nextday-pilot.yaml`。百万参数容量实验使用
`configs/nextday-raw-1m-pilot.yaml`。pilot 先完成数据审计、Logistic 基线和 100 个 batch 的
Colab 吞吐测试，再确定端到端训练预算。日度指标要求至少 80 只股票。已查看过结果的 2024Q4
只作为开发证据，新的模型选择不能重新把它当作未见测试集。
