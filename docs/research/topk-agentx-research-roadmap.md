...
M3 已形成正式停止结论。M4 与 M5 的最近折同口径增量实验已经完成，HGB 组合输入和联合训练都得到正 Rank IC。半衰期、交易门槛、已知风格暴露、任务梯度和标签尺度检查已经完成。标签尺度提高了两折 Rank IC，头部收益没有形成完整跨折增量。下一阶段进入监督位置消融。原始盘口扩容、相邻折 seed 1、2 和 150M 事件流容量消融继续暂停。
...
前四项的正式结论见[事件流信号半衰期与交易转换诊断](eventstream-signal-trading-diagnostics.md)。梯度审计和标签尺度实验也已完成，结果见[事件流多任务梯度审计](eventstream-gradient-audit.md)和[事件流标签尺度实验](eventstream-label-scale.md)。监督位置实验继续只改变一个训练机制。单 seed 需要同时改善两个滚动折的 validation 和 OOS，才补更多 seed 与滚动窗口。容量扩张排在这组验证之后。
...
12. [ ] 完成任务梯度、标签尺度和监督位置的单变量消融。梯度与标签尺度已完成，监督位置训练入口已就绪，待运行两折 seed 0 validation。
...
2. 实现 `all`、`last` 和 `tail_weighted` 三种日级监督模式，现有 z 标签 `all` 结果作为对照。
3. 在两折运行 `last` 和 `tail_weighted` 的 seed 0 validation，按预注册门槛选出一个方案后再开放 OOS。
4. 两折 Rank IC 和极端组收益差都改善后，再补 seed 1、2。没有方案通过时检查日级任务权重和成本感知排序目标。
