...
- L2 事件流已经完成 2025 年 8 月至 12 月的 103 个交易日打包和 A100 输入基准。固定窗口缓存与正式训练工作流已经实现，真实 seed 0 物化、上传和短恢复验收仍待完成。
...
M3 已形成正式停止结论，下一项本地研究是 M4 的 HGB 与 LambdaMART 同口径比较。事件流 T0 基础设施继续准备，门槛通过后再运行 `capacity100m` seed 0。原始盘口扩容继续停止，后续大容量实验只用于检验事件流表征及其下游增量。
...
截至 2026-08-16，本机没有可用 CUDA GPU。Google Drive 总额为 200 GiB，当前约使用 98.1 GiB，剩余约 100.5 GiB，只上传了 2025 年 8 月的 68.58 GB benchmark pack。完整五个月 pack 约为 313.11 GiB，无法整体放入现有 Drive。正式训练改用每个 seed 约 25 GiB 的固定窗口缓存。缓存保留 float32 特征和原始标签，并绑定源清单、配置、日期、seed 与逐文件 SHA-256。

1. 先完成 T0 基础设施门槛。生成并上传真实 seed 0 固定窗口缓存，核对远端文件，再完成一次短训练与 checkpoint 恢复测试。
...
固定窗口缓存与正式训练工作流已经实现。完成真实 seed 0 的物化、上传和短恢复测试后，S0b 才能记为通过。
...
1. 启动 M4，在同一 M3 v2 数据指纹下比较 HGB 与 LambdaMART，并补齐 NDCG、Precision 和风险暴露诊断。
2. 完成事件流固定窗口缓存的真实 seed 0 物化、上传与短恢复验收。
3. 验收通过后，在远端 GPU 运行 `capacity100m` 最近折 seed 0。通过预设门槛后补 seed 1 和 2，失败结果同样进入实验记录。
4. 把通过门槛的 100M 事件流 checkpoint 冻结为 embedding，接入 M4 的相同下游合同。只有事件流信号门槛或冻结 embedding 迁移门槛通过，才运行 `probe150m` 受控容量消融。
