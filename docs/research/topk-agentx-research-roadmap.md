...
M3 已形成正式停止结论。M4 与 M5 的最近折同口径增量实验已经完成，HGB 组合输入和联合训练都得到正 Rank IC。相邻滚动折 seed 0 继续得到正 Rank IC，交易价值和头部稳定性仍需补证。下一阶段先检查信号半衰期、错峰持有、交易门槛和风险暴露，再运行训练目标消融。原始盘口扩容继续停止，150M 事件流容量消融等待这些结果。

## 外部对比带来的验证队列

[外部 L2 研究项目对比](external-l2-research-comparison.md) 已把外部稿件中的判断分成仓库事实、外部项目自述、机制假设和研究决策。当前采纳的顺序如下：

1. `EVT-HALFLIFE-001` 计算最近折和相邻滚动折的 `IC(1)` 至 `IC(10)` 与按建仓日分组的组合收益
2. `TRD-STAGGERED-H5-001` 比较每日全量换仓和 H5 五组错峰持有
3. `TRD-RANK-EMA-001` 比较排名 EMA、绝对开仓门槛、换仓收益差门槛和现金仓位
4. `RISK-ATTR-001` 补齐行业、规模、流动性和波动率归因
...
前四项复用已有预测，优先回答现有信号如何转成组合。后三项每次只改变一个训练机制。单 seed 需要同时改善 validation 和相邻 OOS，才补更多 seed 与滚动窗口。容量扩张排在这组验证之后。
...
11. [ ] 完成半衰期、H5 错峰持有、排名平滑、开仓门槛和风险暴露检查。
...
本次决策为 `EXTEND`。相邻滚动折 seed 0 已继续得到正 Rank IC，下一步执行 `EVT-HALFLIFE-001`、`TRD-STAGGERED-H5-001`、`TRD-RANK-EMA-001` 和 `RISK-ATTR-001`。随后再运行任务梯度、标签尺度与监督位置消融。`probe150m` 继续暂缓。
...
2. 补充额外时间窗口，复核 HGB E2 与联合模型的 Rank IC、NDCG、Precision 和成本后主动收益。
3. 根据三 seed 结果评估小规模横截面排序损失或成本感知选模，重点改善 Top-100 头部质量。
4. 跨窗口和交易指标通过后，再评估 `probe150m` 的预算和 seed 0 门槛。
