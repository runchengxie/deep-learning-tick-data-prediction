...
M3 已形成正式停止结论。M4 与 M5 的最近折同口径增量实验已经完成，HGB 组合输入形成当前折内稳定的 Rank IC 增量，交易价值和跨窗口稳定性仍需补证。下一阶段先补风险暴露和一个 seed 的联合端到端小实验。原始盘口扩容继续停止，150M 事件流容量消融等待联合实验与额外时间窗口结果。
...
9. [ ] 运行 `FEAT-EVENTSTREAM-JOINT-001` seed 0，补齐风险暴露并在额外时间窗口复核。
10. [ ] 根据联合实验和跨窗口结果决定是否启动 `probe150m`。
...
当前折通过表征增量的小实验门槛，允许固定 100M 容量运行一个 seed 的联合端到端对照。M5 的完整验收门槛仍需额外时间窗口、风险暴露和成本后主动收益共同支持。联合实验不能改变股票日交集、日期、标签或评估方式，直接对照 frozen E2。`probe150m` 保持等待。
...
M0 至 M3 已完成。M4 与 M5 的最近折冻结表征对照也已完成，当前优先级如下：

1. 补齐行业、规模、波动率和流动性暴露输入。当前代码会在缺少暴露文件时显式标记 `unavailable`，不会用空结果代替诊断。
2. 固定 100M 容量、股票日交集、标签和评估方式，运行 `FEAT-EVENTSTREAM-JOINT-001` 的单 seed 小实验，直接对照 frozen E2。
3. 补充额外时间窗口，复核 HGB E2 与联合模型的 Rank IC、NDCG 和成本后主动收益。
4. 联合实验与跨窗口证据通过后，再评估 `probe150m` 的预算和 seed 0 门槛。
