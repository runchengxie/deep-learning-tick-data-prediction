...
- 联合端到端 seed 0 已完成。validation Rank IC 为 0.05784，2025 年 12 月 OOS Rank IC 为 0.06296，高于 frozen E2 seed 0 的 0.02462 和 0.04333。OOS `NDCG@100` 为 0.54452，`Precision@100` 为 0.24762，Top-100 日均成本后主动收益为 -9.26bp，日均单边换手为 49.91%。
...
M3 已形成正式停止结论。M4 与 M5 的最近折同口径增量实验已经完成，HGB 组合输入和联合 seed 0 都提高了 Rank IC，交易价值和跨窗口稳定性仍需补证。下一阶段补风险暴露、联合 seed 1、2 和额外时间窗口，并检查头部排序目标。原始盘口扩容继续停止，150M 事件流容量消融等待这些结果。
...
截至 2026-08-18，本机没有可用 CUDA GPU。Google Drive 总额为 200 GiB，共享尾盘缓存上传时剩余约 15.2 GiB。完整五个月 pack 约为 313.11 GiB，正式训练改用每个 seed 约 25 GiB 的固定窗口缓存。三个 seed 的缓存、checkpoint 和结果均已完成并通过指纹核对。
...
9. [x] 运行 `FEAT-EVENTSTREAM-JOINT-001` seed 0，并与 frozen E2 使用同一股票日和评估口径。
10. [ ] 补齐联合 seed 1、2、风险暴露和额外时间窗口复核。
11. [ ] 根据多 seed、跨窗口和交易指标结果决定是否启动 `probe150m`。
...
当前折通过表征增量的小实验门槛，已经固定 100M 容量运行一个 seed 的联合端到端对照。M5 的完整验收门槛仍需额外时间窗口、风险暴露和成本后主动收益共同支持。联合实验使用与 frozen E2 相同的股票日交集、日期、标签和评估方式。`probe150m` 保持等待。

联合实验缓存只保存分钟特征、目标、尾盘缓存相对分片和行号，避免复制事件数组。训练加载 seed 0 的固定 checkpoint，先训练新增特征塔和分类头，再以较小学习率解冻事件流主干。正式结果继续输出 validation 与 OOS 的 Rank IC、NDCG、Precision、Top-K 收益和换手率，并记录缓存、checkpoint 和源码指纹。

`FEAT-EVENTSTREAM-JOINT-001` seed 0 已完成。轻量缓存包含 37,497 个模型样本和 40,274 个组合目标，共 17,948,094 字节，数据指纹为 `e4f54a62e4be3f36ac0693db59ebcdb120cd753d2dc36415b8686adaa13c1bb6`。训练源码 revision 为 `da01954b22a1a1506c9e91f8558fcd80bf8184e8`，预训练 checkpoint SHA-256 为 `8632e62bdf4f27383e299c3ff676876d8a1969f6d69ec66a7ce43da24f5255e9`。本地与 Drive 的 7 个最终文件核对一致，2026 locked 数据没有进入训练或评估。

最佳 checkpoint 出现在第 2 个 epoch。validation Rank IC 为 0.05784，OOS Rank IC 为 0.06296。对应 frozen E2 seed 0 为 0.02462 和 0.04333，冻结三 seed 预测均值为 0.02833 和 0.05701。联合模型的 OOS `NDCG@100` 为 0.54452，`Precision@100` 为 0.24762，Top-100 日均单边换手为 49.91%，日均成本后主动收益为 -9.26bp。Rank IC 增量清晰，头部命中率和成本后主动收益没有同步改善。

本次决策为 `EXTEND`。继续验证联合训练的多 seed 和跨窗口稳定性，并补充风险暴露。交易目标优化优先于容量扩张，可以先检查小规模横截面排序损失或成本感知选模。`probe150m` 继续暂缓。
...
M0 至 M3 已完成。M4 与 M5 的最近折冻结表征对照和联合 seed 0 也已完成，当前优先级如下：

1. 补齐行业、规模、波动率和流动性暴露输入。当前代码会在缺少暴露文件时显式标记 `unavailable`，不会用空结果代替诊断。
2. 为联合训练补齐 seed 1、2 的 checkpoint 身份和配置覆盖，再运行两组重复实验。
3. 补充额外时间窗口，复核 HGB E2 与联合模型的 Rank IC、NDCG、Precision 和成本后主动收益。
4. 根据多 seed 结果评估小规模横截面排序损失或成本感知选模，重点改善 Top-100 头部质量。
5. 多 seed、跨窗口和交易指标通过后，再评估 `probe150m` 的预算和 seed 0 门槛。
