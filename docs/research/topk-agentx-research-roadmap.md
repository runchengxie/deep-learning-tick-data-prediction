...
M3 已形成正式停止结论。M4 与 M5 的最近折同口径增量实验已经完成，HGB 组合输入和联合训练都得到正 Rank IC，交易价值和跨窗口稳定性仍需补证。下一阶段补风险暴露和额外时间窗口，并检查头部排序目标。原始盘口扩容继续停止，150M 事件流容量消融等待这些结果。
...
截至 2026-08-18，本机没有可用 CUDA GPU。Google Drive 总额为 200 GiB，联合三 seed 完成后剩余约 10.3 GiB。完整五个月 pack 约为 313.11 GiB，正式训练改用每个 seed 约 25 GiB 的固定窗口缓存。三个 seed 的缓存、checkpoint、冻结 embedding 和联合训练结果均已完成并通过指纹核对。
...
10. [ ] 补齐风险暴露和额外时间窗口复核。
11. [ ] 根据跨窗口和交易指标结果决定是否启动 `probe150m`。
...
本次决策为 `EXTEND`。继续验证跨窗口稳定性并补充风险暴露。交易目标优化优先于容量扩张，可以先检查小规模横截面排序损失或成本感知选模。`probe150m` 继续暂缓。
