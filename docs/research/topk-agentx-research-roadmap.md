...
- 多任务梯度审计和标签尺度实验已完成。每日截面 z 标签把最近折的 validation、OOS Rank IC 提高到 0.11747、0.07446，把相邻折提高到 0.13534、0.07755。最近折的 OOS 极端组收益差从 0.34105% 降到 0.08885%，两折全指标改善门槛没有通过。
- 监督位置实验已完成必要条件检查。最近折 `last` 和 `tail_weighted` 的 validation Rank IC 为 0.07802 和 0.11289，均低于 `all` 的 0.11747。极端组收益差也都较低。正式决定为 `KEEP_ALL`，相邻折与 OOS 保持关闭。
...
M3 已形成正式停止结论。M4 与 M5 的最近折同口径增量实验已经完成，HGB 组合输入和联合训练都得到正 Rank IC。半衰期、交易门槛、已知风格暴露、任务梯度、标签尺度和监督位置检查已经完成。标签尺度提高了两折 Rank IC，最后位置和尾部加权都没有超过全位置基线。下一阶段进入日级任务权重消融。原始盘口扩容、相邻折 seed 1、2 和 150M 事件流容量消融继续暂停。
...
5. [x] `EVT-GRAD-AUDIT-001` 记录各任务对共享主干的梯度范数和夹角
6. [x] `EVT-LABEL-SCALE-001` 比较原始连续收益与每日截面去极值 z 标签
7. [x] `EVT-SUPERVISION-POSITION-001` 比较全部位置、最后位置和尾部加权监督

前四项的正式结论见[事件流信号半衰期与交易转换诊断](eventstream-signal-trading-diagnostics.md)。梯度审计、标签尺度和监督位置实验也已完成，结果见[事件流多任务梯度审计](eventstream-gradient-audit.md)和[事件流标签尺度实验](eventstream-label-scale.md)。监督位置没有增量，下一步只改变日级任务权重。容量扩张排在训练目标验证之后。
...
12. [x] 完成任务梯度、标签尺度和监督位置的单变量消融。监督位置决定为 `KEEP_ALL`。
...
半衰期与交易转换阶段的决定为 `HOLD`。EMA 和换仓收益差门槛降低了换手，H5 五组错峰在 12 月为正，在 11 月为负。两个窗口的成本后主动收益方向没有重复，收益仍集中在少数日期。任务梯度、标签尺度和监督位置消融已完成，下一步检查日级任务权重。相邻折 seed 1、2 与 `probe150m` 继续暂停。完整结果见[事件流信号半衰期与交易转换诊断](eventstream-signal-trading-diagnostics.md)。
...
1. `EVT-LABEL-SCALE-001` 已完成。z 标签提高了两折 validation 和 OOS Rank IC，最近折 OOS 极端组收益差没有改善，暂不补 seed 1、2。
2. `EVT-SUPERVISION-POSITION-001` 已完成必要条件检查，`last` 和 `tail_weighted` 均未超过 `all`，保留 `all` 并保持 OOS 关闭。
3. 审计 z 标签 best checkpoint 的日级梯度强度，再预注册日级任务权重候选和 seed 0 validation 门槛。
4. 任务权重仍然没有形成增量时，再设计小规模成本感知排序目标。
5. 获得带日期的行业分类数据后补齐行业暴露归因。规模、流动性和波动率暴露已经完成第一轮核对。
6. 跨窗口和成本后主动收益形成稳定增量后，再评估 `probe150m` 的预算和 seed 0 门槛。
