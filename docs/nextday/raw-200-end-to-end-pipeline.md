...
参考 notebook 的约定。训练数据在本地从移动硬盘生成，Drive 只保存筛选后的 float16 工作集、配置、checkpoint 和结果，原始 Parquet 不上传。
...
在 `notebooks/nextday_end_to_end_colab.ipynb` 的冒烟模式跑通 Drive 到 Colab 再到 checkpoint 的闭环。确认：
...
先在 Colab notebook 里把 `MODEL_PROFILE` 设为 `capacity_1m` 运行 100 batch，记录吞吐与峰值显存，再分别以 seed 0、1、2 训练。2024Q4 已用于既有 pilot 结果，只作开发诊断。本轮用验证期 Rank IC、Macro F1、训练耗时和跨 seed 波动比较容量增量，该区间不再作为 locked test。只有训练指标提高而验证指标不提高时，停止继续扩容。
