...
```text
使用 2021 年 7 月起点完成 AgentX M3 v2 的 54 个月分钟特征物化
  → 生成正式预测并运行 Top-K 成本诊断
  → 固定 1M/raw-200 候选与 2025 锁定测试条件
  → 运行事件流最近折正式 seed 0
```
...
1. 完成 M3 v2 的 54 个月分钟特征物化和正式 Top-K 诊断
2. 冻结 `1M/raw-200` 候选和 2025 锁定测试条件
3. 运行事件流最近折正式 seed 0，再按门槛决定是否补 seed 1 和 2
4. 保持 2026 锁定，由研究协议和人工审批控制访问
