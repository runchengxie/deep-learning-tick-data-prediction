...
本路线没有把原始 tick 直接进神经网络当作前提。分钟微观结构特征和预计算 embedding 都来自 tick，只是把昂贵计算提前做一次。研究顺序应先验证信息是否存在，再验证原始盘口序列是否提供额外价值。

## 当前工程优先级

当前优先验证深度模型直接接受 tick 和十档盘口并输出次日信号的端到端能力。工程顺序如下：
...
小模型对照仍然保留，只是优先级后移，先打通可复现的端到端链路。当前模型用共享 DeepLOB 编码 2 个 100-event 块，GRU 汇总后同时输出连续超额收益分数和三分类概率，两个损失共同反向传播到原始盘口卷积层。

raw-200 同时保留两个容量档位，86,775 参数默认基线，以及 1,033,383 参数容量扩展。扩展版把卷积通道、Inception 分支和日内 embedding 一起加宽，用来检验模型容量是否构成瓶颈。参数量更大不会自动成为主模型。
...
之后先用 500 tick 加 100 股票检查长窗口增量，再按 200、500、1,000 逐级扩大。每一级都与相同日期、股票池和标签下的分钟模型比较。只有验证期 Rank IC 和稳定性有明确增量，才增加窗口或股票数。

如果原始盘口没有稳定增量，就停止扩展并保留分钟模型。停止本身是有效的研究结论。
