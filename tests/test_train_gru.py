...
from ticknet.nextday.train_gru import (
    MinuteGRUConfig,
...
def _small_config(manifest_path, checkpoint_dir) -> MinuteGRUConfig:
    return MinuteGRUConfig(
        ...
        checkpoint_name="gru-smoke",
        gru_hidden_size=16,
        gru_layers=2,
    )


def test_minute_gru_config_validate_rejects_bad_hidden_size(tmp_path):
    ...
    with pytest.raises(ValueError, match="GRU"):
        replace(config, gru_hidden_size=0).validate()
    with pytest.raises(ValueError, match="GRU"):
        replace(config, gru_layers=0).validate()


def test_minute_gru_train_runs_and_writes_checkpoints(tmp_path):
    manifest_path = _write_minute_manifest(tmp_path, n_shards=4, samples_per_shard=3)
    ...
    assert (checkpoint_dir / "gru-smoke.seed0.best.pt").is_file()
    assert (checkpoint_dir / "gru-smoke.seed0.last.pt").is_file()
    assert (checkpoint_dir / "result.gru-smoke.seed0.json").is_file()


def test_minute_gru_evaluate_best_checkpoints_locked_test(tmp_path):
    manifest_path = _write_minute_manifest(tmp_path, n_shards=4, samples_per_shard=3)
    checkpoint_dir = tmp_path / "ckpt"
    config = _small_config(manifest_path, checkpoint_dir)
    train(config)
...
def test_minute_gru_load_config_unknown_yaml_key_rejected(tmp_path):
    manifest_path = _write_minute_manifest(tmp_path, n_shards=4, samples_per_shard=3)
