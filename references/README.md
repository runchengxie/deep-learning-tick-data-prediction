...
报告研究把分钟级量价序列输入 GRU 做横截面选股，与本项目 `nextday` 的分钟线
（`minute_baseline` / `minute_tcn`）同属一条研究线：先验证分钟特征是否有次日信息，
再对比深度序列模型相对树模型基线的样本外增量，并核算成本后净收益。当前笔记为基于
题目的框架解读，具体方法、特征与数值结论待读原文后回填。
