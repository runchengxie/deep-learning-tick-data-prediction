...
汇总。默认基线有 86,775 个参数；独立的百万参数容量实验把卷积、Inception 和日内 embedding
同步扩展到 1,033,383 个参数，不替换默认配置。模型支持混合精度、梯度累积和断点恢复。
...
作为正式 M3 输入。L2 抽取已改为按股票日连续数组分组，真实 2025-07 新旧分片 SHA-256
完全一致；当前已完成 2025-07 至 2025-12 共 6 个真实月份。剩余全量物化和正式成本矩阵仍待
运行，因此没有新的交易结论。
...
- [docs/h5-rolling-eventstream-roadmap.md](docs/h5-rolling-eventstream-roadmap.md)：H5 Rank IC 主目标、3/1/1 滚动协议与全天事件数据 pilot
