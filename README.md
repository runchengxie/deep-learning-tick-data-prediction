...
项目已经具备可运行的模型、数据转换、论文实验协议、训练恢复、指标汇总和测试。次日
横截面链路同样有可运行的代码、数据适配器、训练、推理和自动化测试。仓库暂未提交真实
FI-2010 的训练结果，因此还没有完成论文数值复现。当前代码可以用于启动 Table I 和
Table II 实验，训练完成后仍需核对各预测跨度的 Accuracy、Precision、Recall 和 F1。
...
新链路与论文复现入口相互独立，使用自己的标签。它把每个股票交易日的最后若干个
