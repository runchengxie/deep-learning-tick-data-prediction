...
截至 2026-08-20，100M 事件流模型已经在最近折三 seed 和第一个相邻滚动折 seed 0 中得到正的 H5 样本外 Rank IC。冻结 embedding 与分钟特征组合、联合端到端训练也得到正的排序增量。每日截面 z 标签的 seed 0 两折实验进一步提高了 validation 和 OOS Rank IC。

现有候选仍未稳定覆盖单边 10bp 交易成本，头部收益也会随月份变化。信号半衰期、交易规则、已具备数据的风险暴露、多任务梯度和标签尺度已经检查完成。标签尺度在最近折的 OOS 极端组收益差没有改善，下一步比较全部位置、最后位置和尾部加权三种日级监督方式。150M 容量实验继续暂缓。
